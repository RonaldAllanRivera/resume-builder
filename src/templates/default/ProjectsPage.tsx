...
export function ProjectsPage({ projects }: ProjectsPageProps) {
  ...
  // Sort projects within each category by order (descending - newest first)
  ...
  const categoryOrder = ['full-stack', 'wordpress', 'automation', 'graphic-design']
  const orderedCategories = categoryOrder.filter((cat) => projectsByCategory[cat])
            ...
            return (
              <section key={category}>
                ...
                <p className="text-sm text-gray-600 mb-6">
                  {categoryProjects.length} project{categoryProjects.length !== 1 ? 's' : ''} in
                  this category
                </p>
